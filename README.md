...
5) Для получения результатов на тестовой выборке запустите файл `python3 main.py --path_to_images path/to/images`. На вход подается директория с изображениями, на выходе - пути к файлам с людьми в очках в файле `results.txt`. Так же информация дублируется в поток вывода, кроме того ведется статистика работы алгоритма обнаружения лица, предоставляемый `dlib`. Кроме того, доступен аргумент `--cnn_detector True`, который запускает детектор, построенный на CNN, в случае ошибок стандартного.
